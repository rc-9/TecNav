...
class TecNav:
    ...
    def __init__(self, df, clinics_info, model):
        ...
        self.tracker = {
            'denver': {'checkin_time': None, 'num_techs': 0,'needed_techs': 0, 'flag': 0, 'available_techs': 0, 'num_movements': 0},
    ...
    def update_tracker(self, indx, row):
        ...
        self.num_techs = row['new_num_techs']
        ...
        self.current_num_techs = self.num_techs + self.tracker[self.location]['num_movements']
        self.current_num_techs_col.append(self.current_num_techs)
        self.tracker[self.location]['num_techs'] = self.current_num_techs
        self.tracker[self.location]['needed_techs'] = self.needed_techs + self.tracker[self.location]['num_movements']
    ...
    def pull_technician(self, pull_from):
        ...
        self.tracker[pull_from]['num_movements'] = -1
        # Update tracker dict by adding 1 to location
        self.tracker[self.location]['num_movements'] = 1
        ...
        print(f'- {self.location.capitalize()}: before count = {self.num_techs} | after count = {self.num_techs + self.tracker[self.location]["num_movements"]}')
        print(f'- {pull_from.capitalize()}: before count = {self.tracker[pull_from]["num_techs"]} | after count = {self.tracker[pull_from]["num_techs"] + self.tracker[pull_from]["num_movements"]}')
        print()
...
def main():
    ...
    # Instantiate logger for application
    logger = logging.getLogger()
    logger.setLevel(logging.DEBUG)
    fh = logging.FileHandler('uc_navigation.log', 'w')
